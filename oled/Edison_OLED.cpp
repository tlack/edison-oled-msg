...
void edOLED::scrollRight(unsigned char start, unsigned char stop, unsigned char speed)
{
	if (stop<start)		// stop must be larger or equal to start
		return;
	scrollStop();		// need to disable scrolling before starting to avoid memory corrupt
	...
	command(0x00);
	command(start);
	command(speed);		// scroll speed frames
	command(stop);
	command(0x00);
	command(0xFF);
	command(ACTIVATESCROLL);
...
void edOLED::scrollLeft(unsigned char start, unsigned char stop)
{
	if (stop<start)		// stop must be larger or equal to start
		return;
	scrollStop();		// need to disable scrolling before starting to avoid memory corrupt
...
void edOLED::scrollLeft(unsigned char start, unsigned char stop, unsigned char speed)
{
	if (stop<start)		// stop must be larger or equal to start
		return;
	scrollStop();		// need to disable scrolling before starting to avoid memory corrupt
